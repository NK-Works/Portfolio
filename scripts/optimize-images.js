...
const dirsToScan = ['public/textures', 'public/assets'];
...
dirsToScan.forEach(d => {
  filesToOptimize = filesToOptimize.concat(getFiles(d));
});
...
async function optimize() {
  for (const file of filesToOptimize) {
    ...
    try {
      // Check if webp already exists (optional, but good for re-runs)
      // For now, we overwrite or recreate.
